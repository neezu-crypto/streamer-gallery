...
// 스트리머 구독제 열람(2026-09, 영구 해금 → 구독형으로 전환) — 모든 스트리머는
// 기본으로 열려있고, 그 스트리머의 첫 이미지가 올라온 시점부터 UNLOCK_DURATION_MS
// 동안 무료로 열람 가능하다. 그 기간이 지나면 만료되고, 별풍선 UNLOCK_BALLOON_COST개
// 후원 인증(관리자 수동 확인)을 거쳐야 다시 UNLOCK_DURATION_MS만큼 연장된다.
// 후원 검증 자체는 soop-stock-market의 "동결 해제(후원)"와 동일하게 완전 수동
// (SOOP API로 실제 후원 내역을 자동 확인하지 않음) — 이 상수는 안내 문구
// 표시용일 뿐, 서버가 실제 후원 여부를 검증하지는 않는다.
const UNLOCK_BALLOON_COST = 10;
const UNLOCK_DURATION_MS = 30 * 24 * 60 * 60 * 1000; // 30일 — 무료체험/갱신 둘 다 이 값 재사용
